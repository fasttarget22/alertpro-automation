...
print("\n" + "="*60)
print("AlertPro CCTV — Real Email Sender (Realistic Emails)")
print("="*60 + "\n")

# Load today's real emails
today = datetime.now().strftime("%Y%m%d")
csv_file = f"smoke_shops_real_day_{today}.csv"
...
print(f"✅ Loaded {len(contacts)} REAL smoke shops\n")
...
try:
    for i, contact in enumerate(contacts, 1):
        try:
            ...
            sent_count += 1
            print(f"[{i}] ✅ {contact['company']} → {contact['email']}")
            server.quit()
            time.sleep(0.5)
            
        except Exception as e:
            failed_count += 1
            print(f"[{i}] ❌ {contact['company']} - {str(e)[:30]}")
...
print(f"\n" + "="*60)
print(f"✅ COMPLETE!")
print(f"   Sent: {sent_count}/{len(contacts)}")
